class Chart {
  ...
  #addEventListeners() {
    ...
    canvas.onmousemove = (evt) => {
      ...
      const pPoints = this.samples.map((s) =>
        math.remapPoint(this.dataBounds, this.pixelBounds, s.features)
      );
  ...
  #getDataBounds() {
    const { samples } = this;
    const x = samples.map((s) => s.features[0]);
    const y = samples.map((s) => s.features[1]);
  ...
  #drawSamples(samples) {
    ...
    for (const sample of samples) {
      ...
      const pixelLoc = math.remapPoint(dataBounds, pixelBounds, features);
      switch (this.icon) {
        case "image":
